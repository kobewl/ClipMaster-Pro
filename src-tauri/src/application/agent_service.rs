...
///
/// **两者必须在同一把锁下**：分成两把锁时，「读槽位」与「记墓碑」之间会释放锁，
/// 取消恰好被抢占在那里就会丢 —— 读到空槽位、随后运行侧装好槽位并把计费请求发出去，
/// 而它写下的墓碑永不命中。完整论证与两个已知角落见 `run_items` 的内联注释。
///
...
struct AgentRunState {
    ...
    /// 预取消墓碑：取消先于注册到达时把号记在这里，晚到的 run 消费它并就地取消
    /// （消费与装入槽位同临界区，见 `run_items`）。
    ///
...
impl AgentService {
    ...
    async fn run_items(
        ...
        // 对本请求自己的号而言，取消与本次运行的交错只有两种位置，两种都以
        // ai_cancelled 收场：
        //   1. 在下面这个临界区**之前**到达 → 它看到的槽位是空的 → 记下墓碑 →
        //      本临界区里的 take_from 命中（槽位因此根本不装）。**前提**是这枚墓碑
        //      还没被容量 8 的 FIFO 淘汰掉，且期间没有别的取消把槽位占上。
        //   2. 在临界区**之后**到达 → 槽位装的就是它的号 → 号匹配 → 它取走并 send →
    ...
    async fn run_inner(
        ...
        config.ensure_key_not_sent_astray()?;
        // 服务名与审计、结果卡片共源，勿与 `action.label()` 混淆。
...
mod tests {
    ...
    /// 命中墓碑的那条路径**从不装槽位**。
    ///
    /// take 排在 install 之前（同一个临界区内），命中就直接返回 —— 命中路径因此
    /// 根本没有槽位可留。这条不变量值得钉死：它就是「不许留下指向已返回函数的悬空
    /// 通道」的等价形式，将来若有人把 install 挪到 take 之前且忘了清，这里会立刻红。
    /// 同时钉住「取消总有审计」在这条路径上照常成立（条目已预填，空条目行不会被
    /// store 丢掉）。
    #[tokio::test]
    async fn tombstone_hit_never_installs_the_slot() {
        ...
        // 取消先到：槽位为空 → 记墓碑（返回 false，契约不变）。
        assert!(!agent.cancel("req-pre"), "空闲时取消没有在途请求被立即取消");
        ...
        let run = agent
            ...
            .await
            .expect("查审计")
            .expect("墓碑取消也要留审计：用户事后要能回答「那次为什么没有结果」");
        assert_eq!(run.error_code.as_deref(), Some("ai_cancelled"));
        assert!(
    ...
    ///
    /// 跑法不靠运气：测试线程先持住状态锁，让**取消方与运行方都卡在锁上排队**，
    /// 再一起放行 —— 谁先拿到锁由调度决定，正是要考的那个竞态。不变量只有一条：
    /// run 必须以 `ai_cancelled` 收场（为何两个方向都成立见 `run_items` 的论证）。
    ///
    /// **诚实的边界**：这条用例钉的是不变量与两个方向的覆盖，不是「能抓住回归」的
    /// 探测器 —— 把消费挪出临界区（拆成两次加锁）后，那条缝只有两条指令宽，卡在锁上
    /// 的两个线程谁先被唤醒基本由调度器的唤醒延迟决定，拿它当探测器会给出虚假的
    /// 安全感。真正的探测器是集成测试
    /// `cancel_never_reports_false_then_true_for_the_same_id`（自旋取消方 + 40 轮，
    /// 且把那条缝放大到 1ms 时该用例必然复现失败）。
