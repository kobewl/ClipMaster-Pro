...
interface SettingsPanelProps {
  ...
  /** 快捷键独立更新后同步父组件的状态（快捷键走独立命令，不经过 onSave） */
    ...
    >
      <div className="w-[340px] rounded-xl bg-white p-5 shadow-2xl dark:bg-neutral-800">
        ...
        <fieldset className="mt-4 rounded-lg border border-black/[0.06] p-3 dark:border-white/[0.08]">
          <legend className="px-1.5 text-[11px] font-medium text-neutral-400">
            数据保留
          </legend>
        ...
        <fieldset className="mt-3 rounded-lg border border-black/[0.06] p-3 dark:border-white/[0.08]">
          ...
          <p className="mt-2 text-[10px] leading-relaxed text-neutral-400">
            点击按钮后按新组合即可修改 · Delete 清除 · Esc 取消
          </p>
        </fieldset>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg px-3 py-1.5 text-xs text-neutral-500 transition-colors hover:bg-black/[0.05] dark:text-neutral-400 dark:hover:bg-white/[0.08]"
          >
            取消
          </button>
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="rounded-lg bg-blue-500 px-4 py-1.5 text-xs font-medium text-white transition-colors hover:bg-blue-600 disabled:opacity-50"
          >
            {saving ? "保存中…" : "保存"}
          </button>
        </div>
