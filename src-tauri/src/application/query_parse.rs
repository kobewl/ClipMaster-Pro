...
///
/// `pub(crate)` 而不是私有：会话的内容词（`session_build::content_terms`）与
/// 检索词共用这一张表 —— 两处各留一份，改一边就是两套口径。
...
/// 「怎么做」不会被「怎么」先吃掉一半。
///
/// `是什么` / `什么是` 是一对**词序相反的疑问短语**（「Redis 是什么」/「什么是 Redis」），
/// 两个都要收。只收 `是什么` 的话，「什么是 mysql 索引」会被当成 3 个词、
/// 放宽阈值从 ⌈2/2⌉=1 抬到 ⌈3/2⌉=2，而该条只命中「索引」1 个词就被整条误筛
/// （实测 0 命中，见 `evals/retrieval/cases.json` 那条用例）—— 用户只是在提问，
/// 却因此搜不到东西。
const QUERY_PREFIXES: &[&str] = &[
    "为什么", "是什么", "什么是", "怎么做", "请问", "帮我", "如何", "怎么", "怎样", "搜索", "关于",
    "找找",
];
...
mod tests {
    ...
    /// `是什么` 与 `什么是` 是词序相反的一对疑问短语，两个都要剥。
    ///
    /// 曾经只收了 `是什么`：「什么是 mysql 索引」被当成 3 个词、放宽阈值从
    /// ⌈2/2⌉=1 抬到 ⌈3/2⌉=2，而该条只命中「索引」1 个词 → 整条被误筛成 0 命中。
    /// 注意「什么是 redis 连接池」两种词数下结果都是 redis-pool（修复前走放宽也留得住），
    /// 所以真正能钉住这个缺口的是「什么是 mysql 索引」那种**只命中一部分词**的查询。
    #[test]
    fn parse_query_strips_both_orders_of_the_question_phrase() {
        assert_eq!(
            parse_query("什么是 redis 连接池"),
            vec!["redis", "连接池"],
            "「什么是」是检索前缀，剥掉后是 2 个内容词（阈值 1），不是 3 个（阈值 2）"
        );
        assert_eq!(
            parse_query("redis 是什么"),
            vec!["redis"],
            "词序反过来（词尾的「是什么」）走停用词表，同样不进检索词集"
        );
    }
